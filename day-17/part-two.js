(() => {
	...
	for (let x = 0; x < input.length; x++) {
		for (let y = 0; y < input.length; y++) {
			if (input[x][y] === '#') {
				activeSpace.set(getKey(x, y, 0, 0), 1);
			}
		}
	}

	const countActiveSpacesAround = (xStart, yStart, wStart, zStart, activeSpace) => {
		let sum = 0;

		for (let z = zStart - 1; z < zStart + 2; z++) {
			for (let w = wStart - 1; w < wStart + 2; w++) {
				for (let x = xStart - 1; x < xStart + 2; x++) {
					for (let y = yStart - 1; y < yStart + 2; y++) {
						if (x === xStart && y === yStart && w === wStart && z === zStart) {
							continue;
						}

						sum += activeSpace.has(getKey(x, y, w, z)) ? 1 : 0;
					}
				}
			}
		}
	...
	for (let cycle = 0; cycle < cycles; cycle++) {
		...
		for (let x = -cycleExpansion; x < xSize + cycleExpansion; x++) {
			for (let y = -cycleExpansion; y < ySize + cycleExpansion; y++) {
				for (let w = -cycleExpansion; w < cycleExpansion; w++) {
					for (let z = -cycleExpansion; z < cycleExpansion; z++) {
						const numberOfActiveSpacesAround = countActiveSpacesAround(x, y, w, z, activeSpace);

						const key = getKey(x, y, w, z);
